...
class GoogleSheetsSync {
    ...
    private function add_hooks() {
        ...
        add_action('wp_ajax_gi_export_duplicate_titles', array($this, 'ajax_export_duplicate_titles')); // 重複エクスポート
    }
    ...
    /**
     * 指定したシート名にデータを追記する（複数行を一度に追記）
     *
     * @param string $sheet_name シート名
     * @param array $rows 追記する行データの配列（二次元配列）
     * @param string $input_option 入力オプション（RAW または USER_ENTERED）
     * @return bool 成功時はtrue、失敗時はfalse
     */
    public function append_to_specific_sheet($sheet_name, $rows, $input_option = 'USER_ENTERED') {
        $access_token = $this->get_access_token();
        if (!$access_token) {
            gi_log_error('Append to Specific Sheet: No access token');
            return false;
        }
        
        $url = self::SHEETS_API_URL . $this->spreadsheet_id . '/values/' . urlencode($sheet_name) . ':append?valueInputOption=' . $input_option;
        ...
        gi_log_error('Appending to specific sheet', array(
        ...
        if (is_wp_error($response)) {
            gi_log_error('Append to Specific Sheet Failed', array(
                'error' => $response->get_error_message(),
                'sheet_name' => $sheet_name
            ));
            return false;
        ...
        gi_log_error('Append to Specific Sheet Response', array(
            'response_code' => $response_code,
            'sheet_name' => $sheet_name,
        ...
        if ($response_code < 200 || $response_code >= 300) {
            gi_log_error('Append to Specific Sheet: Bad response code', array(
                'response_code' => $response_code,
                'response_body' => $response_body
            ));
            return false;
     ...
     * @param string $sheet_title 新しいシートのタイトル
     * @return array|null シートプロパティ（sheetId, title）または失敗時はnull
     */
    public function create_new_sheet($spreadsheet_id, $sheet_title) {
        try {
            $access_token = $this->get_access_token();
            if (!$access_token) {
            ...
            $url = self::SHEETS_API_URL . $spreadsheet_id . ':batchUpdate';
            ...
            gi_log_error('Creating new sheet', array(
                'spreadsheet_id' => $spreadsheet_id,
                'sheet_title' => $sheet_title
            ));
            ...
            gi_log_error('Create New Sheet Response', array(
                'response_code' => $response_code,
            ...
            if (json_last_error() !== JSON_ERROR_NONE) {
                gi_log_error('Create New Sheet: JSON decode error', array(
                    'json_error' => json_last_error_msg(),
                    'response_body' => $response_body
                ));
                return null;
            ...
            gi_log_error('Create New Sheet: Decoded result', array(
                'result_keys' => array_keys($result),
                'has_replies' => isset($result['replies']),
                'replies_count' => isset($result['replies']) ? count($result['replies']) : 0
            ));
            
            if (isset($result['replies'][0]['addSheet']['properties'])) {
                $sheet_props = $result['replies'][0]['addSheet']['properties'];
                gi_log_error('New sheet created successfully', array(
            ...
            gi_log_error('Create New Sheet: Unexpected response format', array(
                'response_body' => $response_body,
            ...
            gi_log_error('Create New Sheet Exception', array(
    ...
    /**
     * 都道府県データの検証と無効な投稿のエクスポート
     */
    public function ajax_export_invalid_prefectures() {
        // エラーレポートを強化
        error_reporting(E_ALL);
        ini_set('display_errors', '0'); // ブラウザには表示しない（JSONレスポンスを壊さないため）
        ini_set('log_errors', '1');
        
        // 独自のエラーハンドラーを設定
        set_error_handler(function($errno, $errstr, $errfile, $errline) {
            $error_msg = "PHP Error [$errno]: $errstr in $errfile on line $errline";
            gi_log_error('Error during export invalid prefectures', array(
                'errno' => $errno,
                'errstr' => $errstr,
                'errfile' => $errfile,
                'errline' => $errline
        ...
        register_shutdown_function(function() {
            ...
            if ($error && in_array($error['type'], array(E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR))) {
                ...
                gi_log_error('Fatal error during export invalid prefectures', array(
                    'error_type' => $error['type'],
                    'error_message' => $error['message'],
                    'error_file' => $error['file'],
                    'error_line' => $error['line']
                ...
                if (!headers_sent()) {
                    status_header(200); // 200を返してJSONをパースできるようにする
                    header('Content-Type: application/json');
                    echo json_encode(array(
                        'success' => false,
                        'data' => $error_msg
        ...
        try {
            gi_log_error('AJAX export invalid prefectures request received', array(
                'user_id' => get_current_user_id(),
                'post_data' => $_POST,
                'spreadsheet_id' => $this->spreadsheet_id,
                'has_access_token_method' => method_exists($this, 'get_access_token'),
                'has_read_sheet_data_method' => method_exists($this, 'read_sheet_data'),
                'has_write_sheet_data_method' => method_exists($this, 'write_sheet_data'),
                'has_create_new_sheet_method' => method_exists($this, 'create_new_sheet')
            ));
            ...
            // 権限チェック
            if (!current_user_can('edit_posts')) {
                gi_log_error('Permission denied for export invalid prefectures', array('user_id' => get_current_user_id()));
                wp_send_json_error('権限がありません');
                return;
            }
            gi_log_error('Permission check passed');
            ...
            gi_log_error('Total posts to check', array('count' => $total_posts));
            
            $invalid_posts = array();
            $batch_size = 50; // 一度に50件ずつ処理
            $page = 1;
            $processed = 0;
            
            // バッチ処理で投稿を取得
            while (true) {
                gi_log_error('Processing batch', array('page' => $page, 'batch_size' => $batch_size));
                ...
                foreach ($batch_posts as $post_id) {
                    ...
                    $prefecture_names = is_array($prefectures) && !is_wp_error($prefectures) ? $prefectures : array();
                    $municipality_names = is_array($municipalities) && !is_wp_error($municipalities) ? $municipalities : array();
                    
                    // 条件1: 都道府県と市町村が両方空白
                    $both_empty = empty($prefecture_names) && empty($municipality_names);
                    
                    // 条件2: 都道府県に47都道府県以外の値が入力されている
                    $has_invalid_prefecture = false;
                    $invalid_prefecture_list = array();
                    
                    foreach ($prefecture_names as $prefecture) {
                        if (!in_array($prefecture, $valid_prefectures)) {
                            $has_invalid_prefecture = true;
                            $invalid_prefecture_list[] = $prefecture;
                        }
                    }
                    
                    // 条件1または条件2に該当する場合
                    if ($both_empty || $has_invalid_prefecture) {
                        // 投稿の詳細情報を取得（必要な場合のみ）
                        $post = get_post($post_id);
                        ...
                        $invalid_posts[] = array(
                            'post_id' => $post_id,
                            'title' => $post->post_title,
                            'prefecture_raw' => implode(', ', $prefecture_names),
                            'municipality_raw' => implode(', ', $municipality_names),
                            'issue_type' => $both_empty ? '都道府県・市町村が空白' : '無効な都道府県',
                            'invalid_values' => implode(', ', $invalid_prefecture_list),
                ...
                // バッチサイズをチェック（unsetする前に）
                $batch_count = count($batch_posts);
            ...
            gi_log_error('Invalid posts found', array(
            ...
            if (empty($invalid_posts)) {
                wp_send_json_success(array(
                    'message' => '無効な都道府県データを持つ投稿は見つかりませんでした。',
                    'count' => 0
                ));
                return;
            }
            ...
            // データ行を準備
            $export_rows = array($headers);
            $check_date = current_time('mysql');
            
            foreach ($invalid_posts as $invalid_post) {
                $export_rows[] = array(
                    $invalid_post['post_id'],
                    $invalid_post['title'],
                    $invalid_post['prefecture_raw'],
                    $invalid_post['municipality_raw'],
                    $invalid_post['issue_type'],
                    $invalid_post['invalid_values'],
                    $invalid_post['post_status'],
                    $invalid_post['post_date'],
                    $invalid_post['post_modified'],
                    $check_date
                );
            }
            
            // 既存の「都道府県」シートをチェック、なければ作成
            gi_log_error('Getting spreadsheet ID');
            $spreadsheet_id = $this->get_spreadsheet_id();
            
            gi_log_error('Attempting to create or update sheet', array(
                'sheet_name' => $sheet_name,
                'spreadsheet_id' => $spreadsheet_id,
                'row_count' => count($export_rows)
            ));
            
            // シートが存在するか確認（読み取りを試みる）
            gi_log_error('Testing if sheet exists');
            $test_read = $this->read_sheet_data($sheet_name . '!A1:A1');
            gi_log_error('Sheet existence test result', array('exists' => ($test_read !== false)));
            
            if ($test_read === false) {
                // シートが存在しないので作成
                gi_log_error('Sheet does not exist, creating new sheet', array('sheet_name' => $sheet_name));
                $create_result = $this->create_new_sheet($spreadsheet_id, $sheet_name);
                
                if (!$create_result) {
                    wp_send_json_error('シートの作成に失敗しました。');
                    return;
                }
                
                gi_log_error('New sheet created successfully', $create_result);
            } else {
                // シートが存在する場合は既存データをクリア
                gi_log_error('Sheet exists, clearing existing data', array('sheet_name' => $sheet_name));
                $this->clear_sheet_range_by_name($sheet_name, 'A:Z');
            }
            
            // データを書き込み
            $range = $sheet_name . '!A1:J' . count($export_rows);
            $write_result = $this->write_sheet_data($range, $export_rows);
            
            if ($write_result) {
                gi_log_error('Export completed successfully', array(
                    'sheet_name' => $sheet_name,
                    'rows_exported' => count($invalid_posts)
                ));
                
                wp_send_json_success(array(
                    'message' => "無効な都道府県データを持つ投稿を「{$sheet_name}」シートにエクスポートしました。",
                    'count' => count($invalid_posts),
                    'sheet_name' => $sheet_name,
                    'spreadsheet_id' => $spreadsheet_id
                ));
            ...
            gi_log_error('Export invalid prefectures failed (Exception)', array(
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ...
            gi_log_error('Export invalid prefectures failed (Error)', array(
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ));
            wp_send_json_error('エクスポートに失敗しました（Fatal Error）: ' . $e->getMessage());
        } catch (Throwable $e) {
            gi_log_error('Export invalid prefectures failed (Throwable)', array(
                'error' => $e->getMessage(),
                'file' => method_exists($e, 'getFile') ? $e->getFile() : 'unknown',
                'line' => method_exists($e, 'getLine') ? $e->getLine() : 'unknown'
            ));
    ...
    /**
     * 指定したシート名でスプレッドシートの範囲をクリア
     */
    public function clear_sheet_range_by_name($sheet_name, $range) {
        $access_token = $this->get_access_token();
        if (!$access_token) {
            gi_log_error('Failed to get access token for clear operation');
            return false;
        ...
        $url = self::SHEETS_API_URL . $this->spreadsheet_id . '/values/' . urlencode($sheet_name . '!' . $range) . ':clear';
        ...
        gi_log_error('Sheet range cleared successfully', array(
            'sheet_name' => $sheet_name,
            'range' => $range
        ));
        return true;
    ...
    /**
     * 指定したシート名でスプレッドシートにデータを書き込み
     */
    public function write_to_sheet_by_name($sheet_name, $range, $values, $create_if_not_exists = true) {
        $access_token = $this->get_access_token();
        if (!$access_token) {
            gi_log_error('Failed to get access token for write operation');
            return false;
        }
        
        // シートが存在するか確認、なければ作成
        if ($create_if_not_exists) {
            $this->create_sheet_if_not_exists($sheet_name);
        }
        
        // 完全な範囲を指定（シート名を含む）
        $full_range = $sheet_name . '!' . $range;
        
        $url = self::SHEETS_API_URL . $this->spreadsheet_id . '/values/' . urlencode($full_range) . '?valueInputOption=RAW';
        ...
        gi_log_error('Writing to sheet by name', array(
            'sheet_name' => $sheet_name,
            'full_range' => $full_range,
            'rows_count' => count($values)
        ));
        ...
        if (is_wp_error($response)) {
            gi_log_error('Write to sheet by name failed', array(
                'error' => $response->get_error_message(),
                'sheet_name' => $sheet_name,
                'range' => $range
            ));
        ...
        $response_code = wp_remote_retrieve_response_code($response);
        if ($response_code < 200 || $response_code >= 300) {
            gi_log_error('Write to sheet by name - bad response code', array(
                'response_code' => $response_code,
                'response_body' => wp_remote_retrieve_body($response),
                'sheet_name' => $sheet_name,
                'range' => $range
        ...
        gi_log_error('Write to sheet by name successful', array(
            'sheet_name' => $sheet_name,
            'range' => $range
        ));
        return true;
    ...
    /**
     * 指定したシート名でスプレッドシートからデータを読み込み
     */
    public function read_from_sheet_by_name($sheet_name, $range) {
        $access_token = $this->get_access_token();
        if (!$access_token) {
            gi_log_error('Failed to get access token for read operation');
            return false;
        }
        ...
        $url = self::SHEETS_API_URL . $this->spreadsheet_id . '/values/' . urlencode($full_range);
        
        gi_log_error('Reading from sheet by name', array(
            'sheet_name' => $sheet_name,
            'full_range' => $full_range
        ));
        ...
        if (is_wp_error($response)) {
            gi_log_error('Read from sheet by name failed', array(
                'error' => $response->get_error_message(),
                'sheet_name' => $sheet_name,
                'range' => $range
            ));
        ...
        if ($response_code !== 200) {
            gi_log_error('Read from sheet by name - bad response code', array(
                'response_code' => $response_code,
                'response_body' => $response_body,
                'sheet_name' => $sheet_name,
                'range' => $range
        ...
        if (isset($data['values']) && is_array($data['values'])) {
            gi_log_error('Read from sheet by name successful', array(
                'sheet_name' => $sheet_name,
                'rows_count' => count($data['values'])
            ));
            return $data['values'];
        }
        
        gi_log_error('Read from sheet by name - no values found', array(
            'sheet_name' => $sheet_name,
            'range' => $range
        ));
        return array();
     ...
     * シートが存在しない場合は作成
     */
    private function create_sheet_if_not_exists($sheet_name) {
        $access_token = $this->get_access_token();
        if (!$access_token) {
            gi_log_error('Failed to get access token for create sheet operation');
            return false;
        }
        
        // スプレッドシートのメタデータを取得してシートの存在を確認
        $url = self::SHEETS_API_URL . $this->spreadsheet_id;
        ...
        if (is_wp_error($response)) {
            gi_log_error('Failed to get spreadsheet metadata', array(
                'error' => $response->get_error_message()
            ));
            return false;
        }
        ...
        $response_body = wp_remote_retrieve_body($response);
        $spreadsheet_data = json_decode($response_body, true);
        
        // シートが既に存在するかチェック
        $sheet_exists = false;
        if (isset($spreadsheet_data['sheets']) && is_array($spreadsheet_data['sheets'])) {
        ...
        if ($sheet_exists) {
            gi_log_error('Sheet already exists', array('sheet_name' => $sheet_name));
            return true;
        }
        
        // シートを作成
        gi_log_error('Creating new sheet', array('sheet_name' => $sheet_name));
        ...
        if (is_wp_error($response)) {
            gi_log_error('Failed to create sheet', array(
                'error' => $response->get_error_message(),
                'sheet_name' => $sheet_name
            ));
            return false;
        ...
        $response_code = wp_remote_retrieve_response_code($response);
        if ($response_code < 200 || $response_code >= 300) {
            gi_log_error('Failed to create sheet - bad response code', array(
                'response_code' => $response_code,
                'response_body' => wp_remote_retrieve_body($response),
                'sheet_name' => $sheet_name
            ));
        ...
        gi_log_error('Sheet created successfully', array('sheet_name' => $sheet_name));
        return true;
    }
    
    /**
     * タクソノミーをエクスポート（AJAX）
     * カテゴリ、都道府県、市町村、タグをそれぞれのシートにエクスポート
     */
    public function ajax_export_taxonomies() {
        // エラーハンドラーを設定
        set_error_handler(function($severity, $message, $file, $line) {
            gi_log_error('PHP Error in ajax_export_taxonomies', array(
                'severity' => $severity,
                'message' => $message,
                'file' => $file,
                'line' => $line
        ...
        register_shutdown_function(function() {
            ...
            if ($error !== null && ($error['type'] === E_ERROR || $error['type'] === E_PARSE || $error['type'] === E_CORE_ERROR || $error['type'] === E_COMPILE_ERROR)) {
                gi_log_error('Fatal error in ajax_export_taxonomies', $error);
            }
        });
        ...
        try {
            ...
            gi_log_error('Starting taxonomy export');
            
            // 登録されているタクソノミーを確認（デバッグ用）
            $all_taxonomies = get_taxonomies(array('object_type' => array('grant')), 'objects');
            $taxonomy_names = array();
            foreach ($all_taxonomies as $tax) {
                $taxonomy_names[] = $tax->name;
            }
            gi_log_error('Available grant taxonomies', array('taxonomies' => $taxonomy_names));
            ...
            foreach ($taxonomies_config as $config) {
                gi_log_error('Exporting taxonomy', array('taxonomy' => $config['taxonomy']));
                ...
                if (is_wp_error($terms)) {
                    gi_log_error('Failed to get terms', array(
                        'taxonomy' => $config['taxonomy'],
                        'error' => $terms->get_error_message(),
                        'error_code' => $terms->get_error_code()
                    ));
                    
                    // エラーでも結果に追加（UIに表示するため）
                    $export_results[] = array(
                        'taxonomy' => $config['label'],
                        'sheet_name' => $config['sheet_name'],
                        'count' => 0,
                        'success' => false,
                        'error' => 'タクソノミーの取得に失敗: ' . $terms->get_error_message()
                    );
                    continue;
                }
                
                gi_log_error('Terms retrieved', array(
                    'taxonomy' => $config['taxonomy'],
                    'count' => count($terms),
                ...
                foreach ($terms as $term) {
                    $parent_name = '';
                    if ($term->parent > 0) {
                        $parent_term = get_term($term->parent, $config['taxonomy']);
                ...
                gi_log_error('Preparing to export', array(
                    'taxonomy' => $config['taxonomy'],
                    'sheet_name' => $config['sheet_name'],
                    'rows_to_export' => count($export_rows),
                    'terms_count' => count($terms)
                ));
                
                // シートをクリア
                $clear_result = $this->clear_sheet_range_by_name($config['sheet_name'], 'A1:H10000');
                gi_log_error('Clear sheet result', array(
                    'sheet_name' => $config['sheet_name'],
                    'result' => $clear_result ? 'success' : 'failed'
                ));
                
                // スプレッドシートに書き込み
                $write_result = $this->write_to_sheet_by_name(
                    $config['sheet_name'],
                    'A1',
                    $export_rows,
                    true
                ...
                gi_log_error('Write sheet result', array(
                    'sheet_name' => $config['sheet_name'],
                    'result' => $write_result ? 'success' : 'failed',
                    'rows_written' => count($export_rows)
                ));
            ...
            // 成功したエクスポートの数をカウント
            $success_count = count(array_filter($export_results, function($result) {
                return $result['success'];
            }));
            
            if ($success_count > 0) {
                wp_send_json_success(array(
                    'message' => "{$success_count}個のタクソノミーをエクスポートしました。",
                    'results' => $export_results,
                    'spreadsheet_id' => $spreadsheet_id
                ));
            } else {
                wp_send_json_error(array(
                    'message' => 'タクソノミーのエクスポートに失敗しました。',
                    'results' => $export_results
                ));
            }
            
        } catch (Exception $e) {
            gi_log_error('Export taxonomies failed (Exception)', array(
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ...
            gi_log_error('Export taxonomies failed (Error)', array(
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
    ...
    /**
     * タクソノミーをインポート（AJAX）
     * スプレッドシートから読み込んで、タクソノミーを作成・更新・削除
     */
    public function ajax_import_taxonomies() {
        // エラーハンドラーを設定
        set_error_handler(function($severity, $message, $file, $line) {
            gi_log_error('PHP Error in ajax_import_taxonomies', array(
                'severity' => $severity,
                'message' => $message,
                'file' => $file,
                'line' => $line
        ...
        register_shutdown_function(function() {
            $error = error_get_last();
            if ($error !== null && ($error['type'] === E_ERROR || $error['type'] === E_PARSE || $error['type'] === E_CORE_ERROR || $error['type'] === E_COMPILE_ERROR)) {
                gi_log_error('Fatal error in ajax_import_taxonomies', $error);
            }
        });
        ...
        try {
            ...
            gi_log_error('Starting taxonomy import');
            ...
            foreach ($taxonomies_config as $config) {
                gi_log_error('Importing taxonomy', array('taxonomy' => $config['taxonomy']));
                
                // スプレッドシートからデータを読み込み
                $sheet_data = $this->read_from_sheet_by_name($config['sheet_name'], 'A1:H10000');
                
                if (empty($sheet_data) || !is_array($sheet_data)) {
                    gi_log_error('No data found in sheet', array('sheet_name' => $config['sheet_name']));
                    $import_results[] = array(
                        'taxonomy' => $config['label'],
                        'sheet_name' => $config['sheet_name'],
                        'created' => 0,
                ...
                if (!is_wp_error($all_terms)) {
                    foreach ($all_terms as $term) {
                        $existing_term_ids[$term->term_id] = true;
                    }
                }
                ...
                foreach ($sheet_data as $row_index => $row) {
                    ...
                    $term_id = !empty($row[0]) ? intval($row[0]) : 0;
                    $name = !empty($row[1]) ? trim($row[1]) : '';
                    $slug = !empty($row[2]) ? sanitize_title($row[2]) : '';
                    $description = !empty($row[3]) ? trim($row[3]) : '';
                    $parent = !empty($row[5]) ? intval($row[5]) : 0;
                    ...
                    // 削除フラグ（名前が"DELETE"または"削除"の場合）
                    if ($name === 'DELETE' || $name === '削除') {
                        if ($term_id > 0) {
                            $result = wp_delete_term($term_id, $config['taxonomy']);
                            if (!is_wp_error($result) && $result) {
                                $deleted++;
                                unset($existing_term_ids[$term_id]);
                            } else {
                                $errors[] = "削除失敗: ID {$term_id}";
                            }
                        }
                        continue;
                    }
                    ...
                    if ($term_id > 0 && term_exists($term_id, $config['taxonomy'])) {
                        // 更新
                        $result = wp_update_term($term_id, $config['taxonomy'], array_merge(
                            $term_args,
                            array('name' => $name)
                        ));
                        ...
                        if (!is_wp_error($result)) {
                            ...
                            $errors[] = "更新失敗: {$name} - " . $result->get_error_message();
                        }
                    } else {
                        // 新規作成
                        ...
                        if (!is_wp_error($result)) {
                            ...
                            // 既に同じスラッグが存在する場合は更新を試みる
                            if ($result->get_error_code() === 'term_exists') {
                                $existing_term_id = $result->get_error_data();
                                $update_result = wp_update_term($existing_term_id, $config['taxonomy'], array_merge(
                                    $term_args,
                                    array('name' => $name)
                                ));
                                ...
                                if (!is_wp_error($update_result)) {
                                    $updated++;
                                    unset($existing_term_ids[$existing_term_id]);
                                } else {
                                    $errors[] = "作成/更新失敗: {$name} - " . $update_result->get_error_message();
                                }
                            } else {
                                $errors[] = "作成失敗: {$name} - " . $result->get_error_message();
                            }
                        }
                    }
                }
                ...
                // メモリをクリア
                unset($sheet_data);
                unset($existing_term_ids);
            }
            
            wp_send_json_success(array(
                'message' => 'タクソノミーのインポートが完了しました。',
                'results' => $import_results
            ));
            
        } catch (Exception $e) {
            gi_log_error('Import taxonomies failed (Exception)', array(
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ));
            wp_send_json_error('インポートに失敗しました: ' . $e->getMessage());
        } catch (Error $e) {
            gi_log_error('Import taxonomies failed (Error)', array(
                'error' => $e->getMessage(),
                'file' => $e->getFile(),
                'line' => $e->getLine(),
                'trace' => $e->getTraceAsString()
            ...
            <div class="gi-sheets-card">
                ...
                <div class="gi-sync-controls">
                    ...
                    <div class="gi-sync-option" style="border-top: 1px solid #ddd; margin-top: 15px; padding-top: 15px;">
                        ...
                        <p class="description">都道府県と市町村が空白の投稿、または無効な都道府県が入力されている投稿を「都道府県」シートにエクスポートします。</p>
                    </div>
                    
                    <div class="gi-sync-option" style="border-top: 1px solid #ddd; margin-top: 15px; padding-top: 15px;">
                        <button type="button" class="button button-secondary" id="export-taxonomies">
                            📊 タクソノミーをエクスポート
                        </button>
                        <p class="description">カテゴリ、都道府県、市町村、タグのマスタデータを各シート（カテゴリ、都道府県_マスタ、市町村_マスタ、タグ）にエクスポートします。投稿数も含まれます。</p>
                    </div>
                    
                    <div class="gi-sync-option" style="margin-top: 10px;">
                        <button type="button" class="button button-primary" id="import-taxonomies">
                            📥 タクソノミーをインポート
                        </button>
                    ...
                    <div class="gi-sync-option" style="margin-top: 10px;">
                        <button type="button" class="button button-primary" id="export-duplicates">
                            📤 重複タイトルをエクスポート
                        </button>
                        <p class="description">
                            <strong>重複している投稿を「重複タイトル」シートにエクスポートします。</strong><br>
                            ✅ 重複グループごとにまとめて表示<br>
                            ✅ 保持推奨・削除候補を自動提案<br>
                            💡 スプレッドシートで整理後、削除候補のE列を「deleted」に変更して同期
                        </p>
                    </div>
                </div>
                ...
                <div id="duplicate-export-result" style="display: none;">
                    <div class="notice">
                        <p id="duplicate-export-message"></p>
                    </div>
                </div>
            </div>
