<?php
/**
 * Category Archive Template for Grant - Yahoo! JAPAN Inspired SEO Perfect Edition
 * カテゴリー別助成金・補助金アーカイブページ - Yahoo!風デザイン・SEO完全最適化版
 * 
 ...
 * - Category-fixed filter (category selector hidden)
...
// カテゴリーメタ情報取得（カスタムフィールドがあれば）
$category_meta = get_term_meta($category_id);
$category_icon = isset($category_meta['category_icon']) ? $category_meta['category_icon'][0] : '';
$category_color = isset($category_meta['category_color']) ? $category_meta['category_color'][0] : '#000000';
...
// 親カテゴリーがあればパンくずに追加
if ($current_category->parent > 0) {
    $parent_category = get_term($current_category->parent, 'grant_category');
    if ($parent_category && !is_wp_error($parent_category)) {
        $breadcrumbs[] = ['name' => $parent_category->name, 'url' => get_term_link($parent_category)];
    }
}

$breadcrumbs[] = ['name' => $category_name, 'url' => ''];
...
// キーワード生成
$keywords = ['助成金', '補助金', $category_name, '検索', '申請', '支援制度', $current_year . '年度'];
                ...
                <div class="widget-content">
                    <ul class="related-categories-list">
                        ...
                        <li class="related-category-item">
                            <a href="<?php echo esc_url(get_term_link($rel_cat)); ?>" class="related-category-link">
                                <?php echo esc_html($rel_cat->name); ?>
                                <span class="related-category-count">(<?php echo number_format($rel_cat->count); ?>)</span>
                            </a>
                        </li>
                        <?php endforeach; ?>
                    </ul>
...
(function() {
    ...
    function init() {
        console.log('🚀 Category Archive v19.0 Initialized:', CATEGORY_SLUG);
    ...
    console.log('✅ Category Archive v19.0 - Fully Loaded');
